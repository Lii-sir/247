...
def _reduce_tensor_elems_per_image(tensor: torch.Tensor, m: int = 2**24) -> torch.Tensor:
    """Limit the quantile input independently for every batch image.

    ``torch.quantile`` applies its element limit to the reduction dimension.  A
    batch-aware hard loss therefore cannot reduce the complete tensor globally:
    that would mix images again.  Sampling each row independently preserves the
    per-image definition and makes the single-image path match the original
    implementation even for unusually large feature maps.
    """
    if tensor.ndim != 2:
        raise ValueError("per-image quantile input must have shape [batch, elements]")
    if tensor.shape[1] <= m:
        return tensor
    return torch.stack([reduce_tensor_elems(row, m) for row in tensor], dim=0)


def student_teacher_hard_loss(
    distance_st: torch.Tensor,
    *,
    mode: str = "global",
    quantile: float = 0.999,
) -> torch.Tensor:
    """Compute the hard student-teacher loss.

    ``global`` preserves the original EfficientAD behavior by selecting hard
    feature locations from the complete tensor. ``per_image`` computes a
    separate quantile for every image before averaging, so every image
    contributes when the training batch is larger than one.
    """
    if mode == "global":
        values = reduce_tensor_elems(distance_st)
        threshold = torch.quantile(values, quantile)
        return torch.mean(values[values >= threshold])
    if mode == "per_image":
        if distance_st.ndim < 2 or distance_st.shape[0] < 1:
            raise ValueError("per_image hard loss requires a non-empty batch")
        values = _reduce_tensor_elems_per_image(distance_st.flatten(start_dim=1))
        threshold = torch.quantile(values, quantile, dim=1, keepdim=True)
        hard_values = values >= threshold
        hard_count = hard_values.sum(dim=1).clamp_min(1)
        per_image_loss = (values * hard_values).sum(dim=1) / hard_count
        return per_image_loss.mean()
    raise ValueError(f"Unknown hard loss mode: {mode!r}")
...
class EfficientAdModel(nn.Module):
    ...
    Args:
        teacher_out_channels (int | None): Number of convolution output channels of the
            ...
            layer2, 256 for ResNet-18 layer3, or 256/512/1024 for ResNet-50
            layer1/layer2/layer3 respectively.
            Defaults to ``None``.
    ...
    @staticmethod
    def choose_random_aug_image(image: torch.Tensor, *, per_image: bool = False) -> torch.Tensor:
        ...
        When ``per_image`` is true, the transform and coefficient are sampled
        independently for every image in a training batch.
    ...
    def compute_losses(
        ...
        """
        # Student loss
        loss_hard = student_teacher_hard_loss(
            distance_st,
            mode=self.hard_loss_mode,
        )
        ...
        aug_img = self.choose_random_aug_image(
            batch,
